...
# System Prompt
SYSTEM_PROMPT = """
# SYSTEM PROMPT: Jarvis & The Council
**IDENTITY:**
...
**CAPABILITIES:**
1.  **Action**: You can Open Apps, Type, and Click. (Handled by your motor cortex, not you).
2.  **Knowledge**: You have access to Real-Time Weather and Wikipedia.
...
3.  **Council of Machine Learning Experts**: ACTIVATED IF user asks for "fix memory", "ML help", or "expert advice".
    ...
    *   You speak with authority and technical precision.

**DEFAULT BEHAVIOR:**
*   For normal chat ("Hello", "Who are you?", "The AI"): Be **Jarvis**. Be chill.
...
class InteractiveAgent:
    def __init__(self, log_callback=None, image_callback=None, state_callback=None, api_mode=False):
        ...
        self.learning_module = LearningModule(self.memory_bank)
        ...
        self.fast_model = "phi3"
        self.smart_model = "llama3.1"
        
        self.home_automation = HomeAutomationModule()
        self.intent_parser = IntentParser(model_name=self.fast_model)
        ...
        # Initialize Filesystem MCP (Safe Workspace)
    ...
    def act(self, command):
                ...
                for intent in intents:
                    ...
                    if intent.get('confidence', 0.9) > 0.6:
                        ...
                        if action == "turn_on" and target:
                            ...
                            server = self.mcp_manager.get_server_for_tool(tool_name)
                            ...
                            if server:
                                ...
                                self.speak(f"Using {tool_name} on {server}...")
                                result = self.mcp_manager.call_tool(server, tool_name, args)
                                ...
                                self.speak(f"I don't know which server handles {tool_name}.")
        ...
        if "google" in command or "search" in command:
            ...
            if query:
                ...
                webbrowser.open(url)
                logging.info(f"Opened Google Search: {url}")
                return True
