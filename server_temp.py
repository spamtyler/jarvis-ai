...
class ChatRequest(BaseModel):
    message: str
    model: str = "qwen2.5:7b"  # Default model for intent parsing
...
async def chat(request: ChatRequest):
    ...
    print(f"📱 Mobile User: {user_message} [Model: {selected_model}]")
    
    # Update agent's model if different
    if hasattr(agent, 'intent_parser') and agent.intent_parser.model != selected_model:
        agent.intent_parser.model = selected_model
